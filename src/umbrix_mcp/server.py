...
@mcp.tool()
async def analyze_indicator(indicator: str, ctx: Context, indicator_type: str = "auto") -> str:
    ...
    try:
        ...
        if result.get('success'):
            ...
            if indicator_info:
                ...
                summary += f"Risk Level: {indicator_info.get('risk_level', 'Unknown')}\n"
...
async def quick_ioc_check(ioc: str, ctx: Context) -> str:
    ...
    try:
        ...
        if result.get('success'):
            ...
            if risk_level in ['high', 'critical'] or status in ['malicious', 'suspicious']:
...
async def threat_intel_chat(question: str, ctx: Context) -> str:
    ...
    try:
        ...
        if result.get('success'):
            ...
            if graph_results:
                summary += "\n\nRelevant Data:"
                for i, result_item in enumerate(graph_results[:3], 1):  # Show up to 3 results
                    summary += f"\n{i}. {result_item}"
