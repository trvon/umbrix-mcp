...
async def test_graph_statistics_tool():
    ...
    with patch('umbrix_mcp.server.umbrix_client') as mock_client:
        ...
        # Verify correct endpoint was called
        mock_http_client.post.assert_called_once_with(
            f"{mock_client.base_url}/v1/tools/graph_statistics",
            json={}
        )
...
async def test_visualize_threat_graph_tool():
    ...
    with patch('umbrix_mcp.server.umbrix_client') as mock_client:
        ...
        mock_http_client.post.return_value = create_mock_response({
            ...
            "data": {
                "results": [
                    {"n.name": "APT28", "r.type": "OPERATES", "m.name": "Malware_X"},
                    {"n.name": "Malware_X", "r.type": "COMMUNICATES_WITH", "m.name": "C2_Server"}
                ]
            }
        })
